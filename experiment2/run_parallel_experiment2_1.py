...
async def main(subprocess_services):
    ...
    for inject in config['injectedSolutionsFraction']:
        for iters in config['mapoModelMaxIterations']:
            for pop_size in config['populationSize']:
                for num_mobiles in config['numMobiles']:
                    ...
                    params = list(map(str, (num_mobiles, pop_size, iters, inject)))
