...
abstract class Experiment(
    ...
    fun start() {
        ...
        fun summarize(controller: MyTestController) {
            val t = Calendar.getInstance().timeInMillis
            table["Delay"]!!.add(TimeKeeper.getInstance().loopIdToCurrentAverage[controller.applications.values.first().loops[0].loopId] ?: Double.POSITIVE_INFINITY)
            table["DC Energy"]!!.add(controller.fogDevices.find { it.name == "cloud" }!!.energyConsumption)
            table["Mobile Energy"]!!.add(controller.fogDevices.filter { it.name.startsWith("m-") }.map { it.energyConsumption }.sum())
            table["Edge Energy"]!!.add(controller.fogDevices.filter { it.name.startsWith("d-") }.map { it.energyConsumption }.sum())
    ...
    protected open fun warmup() {
        experiment(true, 4, 4, 10.0, seed) { }
        experiment(false, 4, 4, 10.0, seed) { }
        experiment(true, 8, 4, 5.0, seed) { }
        experiment(false, 8, 4, 5.0, seed) { }
    }

    protected fun experiment(isCloud: Boolean, totalGateways: Int, numMobilesPerGateway: Int, eegTransRate: Double, seed: Long,
                             summarize: (MyTestController) -> Unit) {
        ...
        val controller = MyTestController(
