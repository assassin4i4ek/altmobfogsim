package api2.dynamic.connection

import api2.dynamic.connection.behaviors.DynamicGatewayConnectionDeviceBehavior
import api2.dynamic.connection.entites.DynamicGatewayConnectionDeviceImpl
    ...
    fun test3() {
        ...
        object: SimEntity("tester") {
            ...
            override fun processEvent(p0: SimEvent) {
                DynamicGatewayConnectionDeviceBehavior.connectChildToParent(gateway, dev)
            }
    ...
    fun test4() {
        ...
        object: SimEntity("tester") {
            ...
            override fun processEvent(ev: SimEvent) {
                when (ev.tag) {
                    1 -> {
                        DynamicGatewayConnectionDeviceBehavior.disconnectChildFromParent(gateway1, dev)
                        send(id, 3.0, 2, null)
                    }
                    2 -> {
                        DynamicGatewayConnectionDeviceBehavior.connectChildToParent(gateway2, dev)
                    }
                }
            }
